    ...
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph) as sess:
            ...
            while True:
                ...
                # marker_img = Image.frombytes('RGB', (sct.width, sct.height), sct.image)
                # image_draw = ImageDraw.Draw(marker_img)
                ...
                #         if scores[0][b] > 0.70:
                #             ymin = int((box[b][0]*sct.height))
                #             xmin = int((box[b][1]*sct.width))
                #             ymax = int((box[b][2]*sct.height))
                #             xmax = int((box[b][3]*sct.width))
                #             image_draw.rectangle([xmin,ymin,xmax,ymax], outline=(255,0,0))
                #             x_avr = (xmin + xmax)/2
                #             y_avr = (ymin + ymax)/2
